...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // expif
      char dummy7[sizeof (IFstmsAST*)];

      // external
      // proto
      char dummy8[sizeof (PrototypeAST*)];

      // program
      // top
      char dummy9[sizeof (RootAST*)];
      ...
      // vardefs
      char dummy12[sizeof (std::vector<BindingAST*>)];
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_initexp = 27,                          // initexp
        S_top = 28,                              // top
        S_definition = 29,                       // definition
        S_external = 30,                         // external
        S_proto = 31,                            // proto
        S_globalvar = 32,                        // globalvar
        S_idseq = 33,                            // idseq
        S_stmts = 34,                            // stmts
        S_stmt = 35,                             // stmt
        S_assignment = 36,                       // assignment
        S_block = 37,                            // block
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_expif: // expif
        value.move< IFstmsAST* > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, IFstmsAST*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const IFstmsAST*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_expif: // expif
        value.template destroy< IFstmsAST* > ();
        break;
    ...
    // Implementation of make_symbol for each token kind.
      ...
      static
      symbol_type
      make_EQUAL_if (location_type l)
      {
        return symbol_type (token::TOK_EQUAL_if, std::move (l));
      }
#else
      static
      symbol_type
      make_EQUAL_if (const location_type& l)
      {
        return symbol_type (token::TOK_EQUAL_if, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      symbol_type
      make_LESS_if (location_type l)
      {
        return symbol_type (token::TOK_LESS_if, std::move (l));
      }
#else
      static
      symbol_type
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_expif: // expif
        value.copy< IFstmsAST* > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_expif: // expif
        value.move< IFstmsAST* > (YY_MOVE (s.value));
        break;
